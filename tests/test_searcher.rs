...
#[test]
fn test_extract_words_static() {
    let words = "The <cat> is (in the) </bag>".extract_words();
    assert_eq!(vec!["The", "cat", "is", "in", "the", "bag"], words);
}

#[test]
fn test_extract_words_dynamic() {
    let text = String::from("The <cat> is (in the) </bag>");
    let words = text.extract_words();
    assert_eq!(vec!["The", "cat", "is", "in", "the", "bag"], words);
}

#[test]
fn test_extract_words_extra() {
    let words = "Кво стаа, Пешо?".extract_words();
    ...
    let words = "   *-~spaces~-* in <<<places>>>   ".extract_words();
